...
function TruckListSideBar(props) {
    ...
    return (
        <>
        <Input  placeholder='Search...' style={{width: "100%"}} onChange={handleOnSearch} value={props.searchValue}/>
        <Container style={{overflow: 'auto', flexGrow: '1', marginTop: 10}}>

          <List selection verticalAlign='middle' size="large">
            {
              props.trucks.filter(filtertruck => {
                return filtertruck.name.includes(props.searchValue) || filtertruck.cuisineType.includes(props.searchValue)
              }).map((t, index) => {
                ...
                return(<List.Item key={index} onClick={(e) => {
                    if(t.departureTime > Date.now()) {
                        ...
                        props.RecenterMap({lat: coordinates[0], lng: coordinates[1]})
                    }
                }}>
        ...
        <Segment style={milesRadiusSegmentStyle}>
          <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10}}>
            ...
            <input type="range" min="1" max='10' onChange={props.handleMilesRadiusChange} value={props.milesRadius}/>
          </div>
          
          <Button onClick={()=>{props.RecenterMap(props.myLocation)}} primary>Go to My Location</Button>
        </Segment>
        </>
    )
}
